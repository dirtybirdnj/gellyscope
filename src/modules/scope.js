// Scope Tab Module - SVG Explorer
// Handles SVG file upload, display, tree navigation, zoom/pan, and crop controls

import { debugLog } from './shared/debug.js';
import { state, setState } from './shared/state.js';
import { formatBytes, escapeHtml } from './shared/utils.js';
...
async function handleFile(file) {
  try {
    ...
    if (result.success) {
      ...
      setState({ currentSVGData: result.data });
      debugLog('SVG data:', state.currentSVGData);
...
function createTreeNode(node) {
  ...
  contentDiv.addEventListener('click', (e) => {
    ...
    setState({ currentSelectedElement: node });
...
function setupSVGControls() {
  ...
  fitBtn.addEventListener('click', () => {
    setState({ viewMode: 'fit', zoomLevel: 1 });
  ...
  fullBtn.addEventListener('click', () => {
    setState({ viewMode: 'full', zoomLevel: 1 });
  ...
  zoomInBtn.addEventListener('click', () => {
    setState({ zoomLevel: Math.min(state.zoomLevel + 0.1, 10) });
    applyZoom();
  });

  // Zoom out
  zoomOutBtn.addEventListener('click', () => {
    setState({ zoomLevel: Math.max(state.zoomLevel - 0.1, 0.1) });
    applyZoom();
  });
  ...
  function getTransformString() {
    let transforms = [`scale(${state.zoomLevel})`];
...
function createCropLine(orientation, position) {
  ...
  document.addEventListener('mousemove', (e) => {
    ...
    if (orientation === 'horizontal') {
      ...
      if (position === 'top') {
        setState({ cropTop: Math.min(clamped, state.cropBottom - 0.05) });
      } else {
        setState({ cropBottom: Math.max(clamped, state.cropTop + 0.05) });
      }
    } else {
      ...
      if (position === 'left') {
        setState({ cropLeft: Math.min(clamped, state.cropRight - 0.05) });
      } else {
        setState({ cropRight: Math.max(clamped, state.cropLeft + 0.05) });
      }
    }
...
function updateCropBounds() {
  ...
  topLine.style.top = `${state.cropTop * 100}%`;
  bottomLine.style.top = `${state.cropBottom * 100}%`;
  leftLine.style.left = `${state.cropLeft * 100}%`;
  rightLine.style.left = `${state.cropRight * 100}%`;

  bounds.style.left = `${state.cropLeft * 100}%`;
  bounds.style.top = `${state.cropTop * 100}%`;
  bounds.style.width = `${(state.cropRight - state.cropLeft) * 100}%`;
  bounds.style.height = `${(state.cropBottom - state.cropTop) * 100}%`;
}
...
export function initScopeTab() {
  debugLog('=== Initializing Scope Tab ===');

  const uploadBtn = document.getElementById('uploadBtn');
  const fileInput = document.getElementById('fileInput');
  const canvasContent = document.getElementById('canvasContent');

  if (!uploadBtn || !fileInput || !canvasContent) {
    console.error('Missing required elements for Scope tab initialization');
    return;
  }
  ...
  debugLog('Scope tab initialized successfully');
}
